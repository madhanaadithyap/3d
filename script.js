  ...
  loaderGLTF.load(
    ...
    (gltf) => {
      ...
      if (selectedCharacter === "./runn.glb") {
        player.scale.set(2.75, 2.75, 2.75);
      }

      if (
        selectedCharacter === "./cin.glb" ||
        selectedCharacter === "./inc.glb"
      ) {
...
function endGame() {
  ...
  finalScoreEl.innerText = `Final Score: ${Math.round(
    score
...
function spawnObstacle() {
  ...
  const color = new THREE.Color().setHSL(colorHue, 1, 0.25);
...
function checkCollisions() {
  ...
  function showCoinSpark(x, y) {
    const spark = document.createElement("div");
    spark.classList.add("coin-spark");
    spark.innerText = "+50";
  ...
  for (let i = coins.length - 1; i >= 0; i--) {
    ...
    if (c.position.distanceTo(player.position) < 1.0) {
      const screenPos = new THREE.Vector3().copy(c.position).project(camera);
...
function animate() {
  ...
  if (player) {
    if (camGameOver) {
      const closeOffset = new THREE.Vector3(5, 0, -2);
  ...
  if (gameRunning) {
    spawnTimer++;
    if (
      spawnTimer % Math.max(10, Math.floor(spawnInterval / (speed + 0.5))) ===
      0
    ) {
